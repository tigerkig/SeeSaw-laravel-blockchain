...
class RegisterController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if (empty(Setting::getValue('enforce_email_verification', 1))) {
            return redirect()->route('user.token', [
				'token_buy' => $request->input('token-buy'),
				'crypto_select' => $request->input('crypto-select')
			]);
        } else {
    ...
    protected function create(array $data)
    {
        ...
        $user = User::create([
            ...
            'email' => $data['email'],
            'mobile' => $data['mobile'],
            'nationality' => strip_tags($data['nationality']),
