...
class IcoStage extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['name', 'start_date', 'end_date', 'total_tokens', 'base_price', 'display_mode', 'liquidity'];
    ...
    public static function token_add_to_account($trnx, $stage_action = '', $user_action = '')
    {
        ...
        if ($stage_action == 'add' || $stage_action == 'sub') {
            if ($stage_action == 'add') {
                if ($stage->price_type == 'dynamic') {
                    $stage->liquidity = number_format(((double) $stage->liquidity + $tnx_amount), max_decimal(), '.', '');
                    $stage->sales_token = number_format(
                        ((double) $stage->sales_token + $tnx_tokens),
                        min_decimal(), '.', ''
                    );
                    $stage->sales_amount = number_format(((double) $stage->sales_amount + $tnx_amount), max_decimal(), '.', '');
                    $stage->base_price = number_format(
                        ((double) $stage->liquidity / ($stage->total_tokens - $stage->sales_token)),
                        max_decimal(), '.', ''
                    );
                } else {
        ...
        if ($user_action == 'add' || $user_action == 'sub') {
            if ($user_action == 'add') {
                if ($stage->price_type == 'dynamic') {
                    $user->tokenBalance = number_format(
                        ((double) $user->tokenBalance + $tnx_tokens),
                        min_decimal(), '.', ''
                    );
                    $user->contributed = number_format(
                        ((double) $user->contributed + $tnx_amount),
                        max_decimal(), '.', ''
                    );
                } else {
