...
class TokenCalculate
{
     ...
     * @return void
     */
    public function calc_token($token, $output = 'total', $transaction = null)
    {
        $return = 0;
        $cost = [];
        ...
        if (!empty($transaction)) {
            ...
            $price = $transaction->base_currency_rate;
        }

        // Price Calculation
        $base = $token * $price;
        $cost['base'] = round($base, max_decimal());
        ...
        if (active_stage()->price_type == 'dynamic') {
            $constant_product = $total_token * $liquidity;
            $liquidity += $base;
            $actual_tokens_recv = $constant_product / $liquidity;

            $total_token = round($actual_tokens_recv, min_decimal());
        }
