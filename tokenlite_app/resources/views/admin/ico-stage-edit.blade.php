                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="input-item input-with-label">
                                           ...
                                           <div class="input-wrap">
                                            <input class="input-bordered" type="number" min="1" id="total_tokens" name="total_tokens" value="{{ $ico->total_tokens }}" required  >
                                            </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="input-item input-with-label">
                                            <div class="input-wrap">
                                                <input type="checkbox" class="input-switch switch-toggle" data-switch="liquidity-input" id="price_type" {{($ico->price_type != 'dynamic')?'':'checked'}} name="price_type">
                                                <label for="price_type">Dynamic Price</label>
                                            </div>
                                            <span class="input-note">Static or dynamic base price based on liquidity</span>
                                        </div>
                                    </div>
                                    <div class="col-md-6 switch-content liquidity-input">
                                        <div class="input-item input-with-label">
                                            <label class="input-item-label">Starting Liquidity</label>
                                           <div class="input-wrap">
                                            <input class="input-bordered" type="number" min="0" id="liquidity" name="liquidity" value="{{ _format(['number' => $ico->liquidity, 'decimal' => 8]) }}" {{($ico->price_type != 'dynamic')?'disabled':'required'}} {{((date('Y-m-d H:i:s') >= $ico->start_date) && (date('Y-m-d H:i:s') <= $ico->end_date) && (get_setting('actived_stage') == $ico->id) && ($ico->status != 'paused'))?'readonly':''}}>
                                            </div>
                                            <span class="input-note">Define your token liquidity. This number will define how much the price is calculated at the start, and how the value of the token can grow. Cannot be changed while a stage is running.<em class="ti ti-help-alt" title="Support up-to 8 decimals but recommended to use up-to 6 decimals." data-toggle="tooltip"></em></span>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="input-item input-with-label">
                                           ...
                                           <div class="input-wrap">
                                            <input class="input-bordered" type="number" min="0" id="base_price" name="base_price" value="{{ _format(['number' => $ico->base_price, 'decimal' => 8]) }}" {{($ico->price_type != 'dynamic')?'required':'readonly'}}>
                                            </div>
                                    ...
                                    <div class="col-12">
                                        <div class="input-item input-with-label">
                                            ...
                                            <div class="row guttar-15px align-items-center">
                                                <div class="col-12">
                                                    <div class="input-wrap">
                                                        <input type="checkbox" class="input-switch" id="is_tracked" {{!empty($ico->is_tracked)?'checked ':''}}name="is_tracked">
                                                        <label for="is_tracked">Enable Tracking</label>
                                                    </div>
                                                    <span class="input-note">Enable this in order to include this stage into the price chart</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@push('footer')
<script>
    let calculateBasePrice = function() {
        let liquidity = parseFloat($('#liquidity').val());
        let total_tokens = parseFloat($('#total_tokens').val());
        if (liquidity > 0) {
            return (liquidity / total_tokens).toFixed(6);
        } else {
            return 1;
        }
    }
    let updateBasePrice = function() {
        let new_price = calculateBasePrice();
        $('#base_price').val(new_price);
    }
    $(document).ready(function() {
        $('#liquidity').on('change textInput input', function(event) {
            if ($('#price_type').is(":checked")) {
                updateBasePrice();
            }
        });
        $('#price_type').change(function(event) {
            if ($('#price_type').is(":checked")) {
                $('#base_price').attr('readonly', true);
                $('#base_price').removeAttr('required');

                $('#liquidity').attr('required', true);
                $('#liquidity').removeAttr('disabled');
                $('.liquidity-input').show();
            } else {
                $('#liquidity').attr('disabled', true);
                $('#liquidity').removeAttr('required');

                $('#base_price').removeAttr('readonly');
                $('#base_price').attr('required', true);
                $('.liquidity-input').hide();
            }
        });
    });
</script>
