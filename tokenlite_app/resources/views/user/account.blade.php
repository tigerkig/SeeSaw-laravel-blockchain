...
@section('content')
@push('header')
<link rel="stylesheet" href="{{ asset('assets/css/intl-tel-input.css') }}">
@endpush
            ...
            <div class="tab-pane fade show active" id="personal-data">
                <form class="validate-modern" action="{{ route('user.ajax.account.update') }}" method="POST" id="nio-user-personal" autocomplete="off">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="input-item input-with-label">
                                <label for="mobile-input" class="input-item-label">{{__('Mobile Number')}}</label>
                                <div class="input-wrap">
                                    <input class="input-bordered" type="text" id="mobile-input" name="mobile" placeholder="{{ __('Enter Mobile Number') }}" value="{{ $user->mobile }}">
                                </div>
                            </div>{{-- .input-item --}}
                        </div>
                        <div class="col-md-6">
...
<div class="content-area card">
    <div class="card-innr">
        ...
        <p>{!! __("Two-factor authentication is a method for protection of your account. When it is activated you are required to enter not only your password, but also a special code. You can receive this code in mobile app. Even if a third party individual gains access to your password, they still won't be able to access your account without the 2FA code.") !!}</p>
            ...
            <div class="popup-body">
                ...
                <form class="validate-modern" action="{{ route('user.ajax.account.update') }}" method="POST" id="nio-user-2fa">
                    ...
                    <div class="pdb-1-5x">
                        ...
                        <p><strong>{{ __('Step 2:') }}</strong> {{ __('Scan the below QR code using your Google Authenticator app, or you can add the account manually.') }}</p>
                        ...
                        <p class="text-danger"><strong>{{ __('Note:') }}</strong> {{ __('If you lost your phone or uninstall the Google Authenticator app, then you will lose access to your account.') }}</p>
                    </div>
                    @else
                    <div class="pdb-1-5x">
                        <div class="input-item">
...
<script src="{{ asset('assets/js/intl-tel-input.js').css_js_ver() }}"></script>
<script type="text/javascript">
