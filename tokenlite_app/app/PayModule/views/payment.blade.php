...
<div class="popup-body">
    <div class="popup-content">
        <form class="validate-modern" action="{{ route('user.ajax.payment') }}" method="POST" id="online_payment">
            @csrf
            <input type="hidden" name="pp_token" id="token_amount" value="{{ to_num($data->token, 'max') }}">
            ...
            @if($pm_check)
                ...
                <ul class="d-flex flex-wrap align-items-center guttar-30px">
                    <li>
						<button type="submit" class="btn btn-alt btn-primary payment-btn"> {{ __('Buy Token Now') }} <em class="ti ti-arrow-right mgl-2x"></em></button>
					</li>
                </ul>
