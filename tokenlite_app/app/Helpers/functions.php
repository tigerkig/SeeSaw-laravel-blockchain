...
/* @function token()  @version v1.0  @since 1.0 */
if (!function_exists('token_price_graph_time')) {
    function token_price_graph_time($minutes = 0)
    {
        if ($minutes <= 60) {
            return '1 Hour';
...
if (!function_exists('to_num_token') && function_exists('to_num')) {
    function to_num_token($num, $decimal='', $thousand=',')
    {
        if (empty($decimal)) {
            if (active_stage()->price_type == 'dynamic') {
                $decimal = 'min';
            } else {
                $decimal = 'zero';
            }
...
if (!function_exists('tracked_stages')) {
    function tracked_stages()
    {
        return IcoStage::where(
            'is_tracked', '=', '1'
        )->orderBy(
            'start_date',
            'DESC'
...
if (!function_exists('is_active_referral_system')) {
    ...
    {
        return get_setting('referral_system', 0) == 1;
    }
}
...
/* @function is_page_role()  @version v1.1  @since 1.0.3 */
if (!function_exists('is_page_role')) {
    function is_page_role()
    {
        $user = (auth()->check()) ? auth()->user() : false;
        $user_role = ($user) ? $user->role : false;

        if($user_role == "subadmin" || $user_role == "operator" || $user_role == "accountant")
            return false;
        return true;
    }
}

/* @function is_user_accountant()  @version v1.1  @since 1.0.3 */
if (!function_exists('is_user_accountant')) {
    function is_user_accountant()
    {
        $user = (auth()->check()) ? auth()->user() : false;
        $user_role = ($user) ? $user->role : false;
...
/* @function is_add_change()  @version v1.1  @since 1.0.3 */
if (!function_exists('is_add_change')) {
    function is_add_change()
    {
        $user = (auth()->check()) ? auth()->user() : false;
        $user_role = ($user) ? $user->role : false;
...
if (!function_exists('style_theme')) {
    function style_theme($panel='base', $ver=true) {
        ...
        $stylesheets = [
            ...
            'branding' => 'assets/css/branding.css',
...
if (!function_exists('currency_to_icon')) {
    function currency_to_icon_class($currency)
    {
        $currency = strtolower($currency);
        $defaults = [
            'usd' => ['aud', 'cad', 'nzd', 'nad', 'kes', 'zar', 'clp', 'cop', 'jmd'],
            'gbp' => ['egp'],
            'busd' => ['busderc20'],
            'bnb' => ['bnbbsc', 'bnbmainnet'],
            'usdt' => ['usdttrc20', 'usdterc20'],
            'unknown' => ['dot', 'shib', 'axs', 'floki', 'sand', 'mana']
        ];
        foreach ($defaults as $target_icon => $values) {
            if (in_array($currency, $values)) {
                if ($target_icon == 'unknown') {
                    return 'coins';
                }
                return 'sign-' . $target_icon;
            }
        }
