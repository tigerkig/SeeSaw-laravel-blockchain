...
class User extends Authenticatable // implements MustVerifyEmail
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'mobile', 'password', 'lastLogin', 'role', 'nationality',
    ];
    ...
    */
    ...
    public static function top_countries()
    {
        ...
        $top_spenders = self::select(DB::raw('
            nationality as country, 
            count(*) as total_users, 
            sum(contributed) as total_contributed, 
            sum(tokenBalance) as tokens
        ...
        $totals = self::select(DB::raw('count(*) as total, sum(contributed) as contributed'))->first();

        foreach ($top_users as $row) {
            $data['users'][] = [
                'country' => !empty($row->country) ? $row->country : 'Unknown',
                'percent' => !empty($row->total_users) && !empty($totals->total) ? round(($row->total_users / $totals->total)*100) : 0,
                'users' => $row->total_users,
                'contributed' => round($row->total_contributed),
        ...
        foreach ($top_spenders as $row) {
            $data['spenders'][] = [
                'country' => !empty($row->country) ? $row->country : 'Unknown',
                'percent' => !empty($row->total_contributed) && !empty($totals->contributed) ? round(($row->total_contributed / $totals->contributed)*100) : 0,
                'users' => $row->total_users,
                'contributed' => round($row->total_contributed),
                'tokens' => round($row->tokens),
            ];
